"""Optimized WSA-Pretrain inference backend.

This backend uses the same checkpoint and public policy type as
``wsa_pretrain``. It skips the WAN video branch and accelerates action
...
from lerobot.policies.wsa_pretrain.modeling_wsa_pretrain import (
    WSAPretrain,
...
class WSAPretrainOptimized(WSAPretrain):
    ...
    def __init__(self, config: WSAPretrainConfig):
        if not config.action_loss_only:
            raise ValueError(
                "WSAPretrainOptimized requires action_loss_only=True so the WAN branch is not loaded."
            )
    ...
    def train(self, mode: bool = True):
        if mode:
            raise RuntimeError("WSAPretrainOptimized is an inference-only backend.")
    ...
    def _capture_graph(
        ...
        if not state.is_cuda:
            raise RuntimeError("WSAPretrain optimized inference backend requires CUDA.")
        ...
        logger.info("Captured WSA-Pretrain optimized CUDA graph for batch=%d prefix_len=%d", *graph_key)
