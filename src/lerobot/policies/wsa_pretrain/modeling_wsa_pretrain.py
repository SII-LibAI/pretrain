...
from lerobot.policies.wsa_pretrain.action_tokens import ensure_qwen35_action_tokens
from lerobot.policies.wsa_pretrain.configuration_wsa_pretrain import WSAPretrainConfig
from lerobot.policies.wsa_pretrain.wan_model import WanVideoModel
from lerobot.policies.wsa_pretrain.wan.modules.model import sinusoidal_embedding_1d
from lerobot.policies.wsa_pretrain.wan.utils.fm import FlowMatchScheduler
...
class WSAPretrainWithExpertModel(nn.Module):
    """Qwen3_5 model with action expert for WSAPretrain."""
...
class WSAPretrain(nn.Module):

    def __init__(self, config: WSAPretrainConfig):
        super().__init__()
        self.config = config
        ...
        self.qwen3_5_with_expert = WSAPretrainWithExpertModel(
    ...
    def gradient_checkpointing_enable(self):
        ...
        logging.info("Enabled gradient checkpointing for WSAPretrain model")
    ...
    def gradient_checkpointing_disable(self):
        ...
        logging.info("Disabled gradient checkpointing for WSAPretrain model")
...
class WSAPretrainPolicy(PreTrainedPolicy):
    """Qwen3.5 VLA policy with WAN video auxiliary supervision."""

    config_class = WSAPretrainConfig
    name = "wsa_pretrain"
    ...
    def __init__(self, config: WSAPretrainConfig):
        ...
        if config.inference_backend == "optimized":
            from lerobot.policies.wsa_pretrain.modeling_wsa_pretrain_optimized import (
                WSAPretrainOptimized,
            )

            self.model = WSAPretrainOptimized(config)
        else:
            self.model = WSAPretrain(config)
